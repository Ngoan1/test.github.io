=== Ecommerce Bookshop ===

Contributors: ThemesPride
Requires at least: 5.0
Tested up to: 6.4
Requires PHP: 5.6
Author URI: https://www.themespride.com/
Theme URI: https://www.themespride.com/themes/free-ecommerce-wordpress-theme/
License: GNU General Public License v3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html
Tags: wide-blocks, block-styles, one-column, two-columns, right-sidebar, left-sidebar, three-columns, four-columns, grid-layout, custom-colors, custom-header, custom-background, custom-menu, custom-logo, editor-style, featured-images, footer-widgets,featured-image-header, full-width-template, sticky-post, theme-options, threaded-comments, flexible-header, blog, e-commerce, education

== Description ==

Ecommerce Bookshop is a theme for Public Library & Book Store. Anyone can use it as it is multipurpose for the app store, book, book author, book publisher, bookseller, book shop, book shopping, book store, digital books, e-learning, e-books, newspaper, online book store, selling e-books, PDFs online, course providers, online courses music and game selling sites. Ecommerce Bookshop has many responsive features such as Clean, user-friendly, responsive, beautiful, accessible, personalization options, testimonial section, and Banner. This theme is compatible with the most popular event calendar. It displays what's coming up and notifies users about the cost of workshops, classes, or other events. Page Builder is used to creating the theme. There are many unique features that this theme supports, including retina-ready, professional and personalization options. You can also customize the options with customization options and multipurpose. It is translation ready. This theme is specifically designed for e-commerce websites. It also allows you to display your business information and the possibility of selling products online. This theme is perfect for e-commerce websites and shops. It can be used to create e-commerce websites for any online store, including fashion, shoes, jewellery, sport, technology, and mobile.

== License ==

Author Writer WordPress Theme, Copyright 2022 ThemesPride
Author Writer is distributed under the terms of the GNU General Public License v2

Ecommerce Bookshop WordPress Theme is child theme of Author Writer WordPress Theme, Copyright 2022 ThemesPride
Ecommerce Bookshop WordPress Theme is distributed under the terms of the GNU GPL

== Changelog ==

= 0.1 =
	. Changed Colors.
	. Changed Font Family.
	. Changed Slider Layout.
	. Changes Product Section.

= 1.1 =
	. Added get-started.
	. Added footer link.
	. Added documentation in customizer.
	. Added responsive media settings.

= 1.2 =
	. Updated sticky in media.

= 1.3 =
	- Added customizer toggle button.
	- Updated customizer CSS.
	- Updated PHP version.

= 1.4 =
	- Added preloader CSS.
	- Remove unwanted settings.

= 1.5 =
	- Added open in new tab setting in social media.
	- Updated function for footer text.
	
= 1.6 =
    - Added slider background image condition.
    - Resolved post block button issue.
    - Resolved post block gallery issue.
    - Resolved pagination style issue.
    - Resolved post layout element issue.
    
= 1.7 = 
  	- Added one click theme color.
    - Updated flexible-header tag.
  	- Resolved post design category blocks issue.
  	- Updated clearing floats css.
  	- Updated custom-logo tag.

= 1.8 =
    - Added customizer.php file. 
    - Added toggle control.
  	- Added show/hide setting for announcement section in contact section.  

= 1.9 =
    - Added fontawsome icon condition.
    - Updated menu icon css.  	 	

= 2.0 =
    - Added single post tag function.

= 2.1 =
    - Added slider text alignment. 
    - Added related posts on single post page.   

= 2.2 =
    - Updated Woocommerce css. 

= 2.3 =
    - Updated Footer css. 

= 2.4 =
    - Added typography.

= 2.5 =
    - Updated theme sidebar css.
    - Updated post css. 

= 2.6 =
    - Added menu font weight setting.

= 2.7 =
    - Updated bootstrap.
    
== Screenshots ==

License: Creative Commons CC0 License
Source: https://pxhere.com/en/license

Image ( Screenshot ), Copyright Pxhere
License: Creative Commons CC0 License
Source: https://pxhere.com/en/photo/665095

Image ( Screenshot ), Copyright Pxhere
License: Creative Commons CC0 License
Source: https://pxhere.com/en/photo/362272

Image ( Screenshot ), Copyright Pxhere
License: Creative Commons CC0 License
Source: https://pxhere.com/en/photo/193340

Image ( Screenshot ), Copyright Pxhere
License: Creative Commons CC0 License
Source: https://pxhere.com/en/photo/1215655

Pxhere Images License: CC0 1.0 Universal (CC0 1.0) 
Source: https://pxhere.com/en/license

License: CC0 1.0 Universal (CC0 1.0) 
Source: https://pxhere.com/en/photo/650710

= Child Theme Changes =

1. Changed Colors.
2. Changed Font Family.
3. Changed Slider Layout.
4. Changes Product Section
