...
if(!class_exists('Woo_Address_Selectbox_Class')) {
    class Woo_Address_Selectbox_Class
    {
        ...
        {
            if (
                isset($_POST['_shipping_phone'])
                && (
                        ('shop_order' === get_post_type( $post_id ) && version_compare(WC()->version, '8.0.0', '<'))
                        || ('shop_order' === OrderUtil::get_order_type( $post_id ) && version_compare(WC()->version, '8.0.0', '>='))
                )
            ) {
